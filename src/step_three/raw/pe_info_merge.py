...
def spark_data_flow():
    ...
    raw_pe_risk_score_df = spark.read.json(
        ("{path}/"
         "pe_feature_risk_score/"
    ...
    tid_pe_risk_score_df = tid_pe_risk_score_df.join(
        raw_basic_df,
        raw_basic_df.bbd_qyxx_id == tid_pe_risk_score_df.bbd_qyxx_id,
        'left_outer'
    ).select(
        tid_pe_risk_score_df.bbd_qyxx_id,
        tid_pe_risk_score_df.company_name,
        tid_pe_risk_score_df.risk_index,
        tid_pe_risk_score_df.risk_composition,
        raw_basic_df.company_county
    )
    tid_pe_risk_score_df = tid_pe_risk_score_df.join(
        county_mapping_df,
        county_mapping_df.code == tid_pe_risk_score_df.company_county,
        'left_outer'
    ).join(
        black_df,
...
def run():
    ...
    os.system(
        ("hadoop fs -rmr " 
         "{path}/"
         "pe_info_merge/{version}").format(path=OUT_PATH, 
                                           version=RELATION_VERSION))    
