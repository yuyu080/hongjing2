...
def get_tags(row):
    def get_ex_trading_exchange_risk(risk_name=u'交易场所行业风险'):
        tags = []
        if row['ex_feature_1'] == 0:
            tags.append(u'无政府批文')
        if row['ex_feature_3'] == 2 or row['ex_feature_3'] == 3:
            tags.append(u'交易权益/期货类品种')
        if row['ex_feature_4'] == 0:
            tags.append(u'无管理规则')
        return {
            risk_name: tags
        }
    
    def get_ex_company_risk(risk_name=u'综合实力风险'):
        tags = []
    ...
    def get_ex_trading_risk(risk_name=u'企业行为风险'):
        tags = []
    ...
    result = {}
    result.update(get_ex_trading_exchange_risk())
