...
def logistic_score():
    ...
    raw_rdd = input_df.rdd.repartition(100).mapPartitions(lambda x: count_logistic(x, weight, zhibiao, sk, b, regression_coefficient))
    # 转成df并重新命名
...
def GBDT_score():
    ...
    feature_rdd = feature_df.rdd.repartition(100).mapPartitions(lambda x: count_gbdt(x, gbdt))
