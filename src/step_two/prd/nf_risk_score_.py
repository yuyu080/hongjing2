...
def change_prob_score(row, quantile_one, quantile_two):
    ...
    if raw_prob >= quantile_one:
        score = (raw_prob - quantile_one) * 40. / (1 - quantile_one) + 50
...
def spark_data_flow():
    ...
    #取5%与50%的分位点
    top_five = int(len(score_distribution) * 0.05)
    top_fifty = int(len(score_distribution) * 0.5)
    #得到分位点
    Y_1 = score_distribution[top_five]
    Y_2 = score_distribution[top_fifty]
    #得到结果
    tid_rdd = raw_rdd.map(
