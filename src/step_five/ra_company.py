...
def run():
    ...
    def addColumns(col_1, col_2):
        return col_1 + col_2

    addColumns_udf = fun.udf(addColumns, tp.StringType())

    prd_df = prd_df.select(
        prd_df.id,
        prd_df.company,
        prd_df.risk_index,
        prd_df.industry,
        fun.when(prd_df.province == prd_df.city, addColumns_udf(prd_df.city, prd_df.area)).otherwise(
            addColumns_udf('province', 'city')).alias(
            'register_area'),
        prd_df.risk_level,
        prd_df.gmt_create,
        prd_df.risk_scan
...
if __name__ == '__main__':
    ...
    confPath = os.path.join(os.path.dirname(os.path.dirname(dir)), 'conf', 'hongjing2.py')
    conf.read(confPath)

    VERSION_LIST = ["20170117", "20170221", "20170315",
    ...
    OUT_PATH = conf.get('step_five', 'OUT_PATH')
