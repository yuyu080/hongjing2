...
def saveMid():
    ...
    input_df = spark.read.parquet(
        ("{path}/"
         ...
         "{version}").format(path=IN_PAHT,
                             version=RELATION_VERSION))
    input_df_logistic = input_df.rdd.map(
        get_datas
    ).toDF().fillna(0.)
    writeCSV(input_df_logistic, '/user/antifraud/hongjing5/test/', 'logistic')
    os.system("hadoop fs -getmerge /user/antifraud/hongjing5/test/logistic /data5/antifraud/qiling/data/logistic.csv")

    input_df_gbdt = input_df.rdd.map(
        get_data_gbdt
    ).toDF()
    writeCSV(input_df_gbdt, '/user/antifraud/hongjing5/test/', 'gbdt')
    os.system("hadoop fs -getmerge /user/antifraud/hongjing5/test/gbdt /data5/antifraud/qiling/data/gbdt.csv")
